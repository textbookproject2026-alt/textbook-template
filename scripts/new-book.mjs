...
//   index.md                  the front page, once, from scripts/seed-index.md
//   the managed files         by running configure.mjs --candidate (optional)
//
// Every book is built by the platform's builder (quartz-book) and served from a
// Cloudflare Pages project in the platform's account, so there is no host to
// choose: the entry always has site.host.builder "quartz-book".
//
// WHAT IT WILL NOT DO
//   Touch registry.json, open a pull request, create a repository, create DNS,
//   install the GitHub App, or create the Pages project. Every one of those is
//   somebody's decision, and three of them are the platform owner's.
//
...
const SHARED_SUFFIXES = ["pages.dev", "workers.dev", "vercel.app", "netlify.app", "github.io"];
...
say("==========================");
say("");
say("A few answers, then a file a platform owner reviews. Nothing here changes");
...
say("");
say("The maintainer's GitHub login: who the platform contacts about this book.");
...
say("");
say("The Cloudflare Pages project the builder deploys this book to, in the");
say("platform's account. Its name is also the free address <project>.pages.dev,");
say("and pages.dev names are global: if Cloudflare gives the project a different");
say("address, the registry entry must use that one.");
const project = await ask("Pages project name:", {
  def: slug,
  validate: (v) => (PROJECT_RE.test(v) ? null : "Lower-case letters, digits and hyphens, at most 58 characters."),
});
say("");
say("The hostname readers visit. A free <project>.pages.dev address is allowed, but");
say("only while the book is a preview: the platform cannot park or redirect a");
say("hostname it does not hold, so the registry refuses to let such a book go live.");
say("A book meant for readers wants a hostname under the portal.");
const domain = await ask("Hostname:", {
  def: bookParent ? `${slug}.${bookParent}` : `${project}.pages.dev`,
  validate: (v) => {
    ...
    if (sharedSuffixOf(v) && v !== `${project}.pages.dev`) return `The only free address this book can have is ${project}.pages.dev.`;
...
const site = {
  domain,
  aliases: [],
  host: { kind: "static", provider: "cloudflare-pages", project, paid_by: "platform", builder: "quartz-book" },
  legacy_origins: [],
  dark: null,
};
...
- The Pages project \`${project}\` (Direct Upload, platform account; before the merge), and DNS plus
  the project's custom domain for \`${site.domain}\` if it is not a \`*.pages.dev\` address.
- A run of \`reconcile\` in \`quartz-book\` after the merge, or the book's first push to wait for.
...
const config = { slug };
// The front page, filled in once. After this it is the author's page, not a
// generated one: configure.mjs never touches it.
const seed = await readFile(join(ROOT, "scripts", "seed-index.md"), "utf8");
const indexText = seed
  .split("__TITLE__").join(title)
  .split("__SUMMARY__").join(summary)
  .split("__SITE_URL__").join(`https://${domain}`)
  .split("__MAINTAINER_YAML__").join(JSON.stringify(maintainerName))
  .split("__MAINTAINER__").join(maintainerName)
  .split("__LICENCE__").join(licence);
const writeIndex = !(await exists(join(ROOT, "index.md")));
...
if (writeIndex) say("  index.md                 the front page, to edit from then on");
say("");
...
if (writeIndex) await writeFile(join(ROOT, "index.md"), indexText);
say("\nWritten.");

// --- optionally render the managed files ------------------------------------

say("");
say("The managed files (README.md, CONTRIBUTING.md, .lycheeignore,");
...
say("     make the book build or resolve: the builder and the services read the registry.");
say("");
