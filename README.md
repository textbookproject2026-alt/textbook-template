...
platform registry, a Cloudflare Pages project in the platform's account, the
GitHub App installed on the repository, and — if the book uses a portal
subdomain — a DNS record. The registry is the one that matters: the builder reads
it at every build and every shared service bakes it in at deploy time, so a book
that is not in the registry does not exist to any of them.

That is why this template is **content plus one renderer**, and not a working
site. The site is built by the platform's builder,
[`quartz-book`](https://github.com/textbookproject2026-alt/quartz-book), the same
way for every book: one design, the graph, Hypothes.is, the Edit / History /
Suggest row and paragraph numbers, all from one place.
